...
export function themeCssPlugin(options = {}) {
  const { themesDir = 'src/themes', output = 'theme.css', defaultPrefix = '--pui' } = options
  ...
  function parseThemeObjectFromFileContent(content) {
    ...
    const cleaned = content.replace(/^import\s+type\s+.*$/gm, '').replace(/^export\s+type\s+.*$/gm, '')
  ...
  function generateCssFromTokens(tokenGroups) {
    ...
    for (const group of tokenGroups) {
      const selector =
        typeof group.className === 'string' && (group.className.startsWith(':') || group.className.startsWith('.'))
          ? group.className
          : `.${group.className}`
      const map = flattenVariables(group.prefix || defaultPrefix, group.variables)
  ...
  return {
    ...
    async buildStart() {
      ...
      const pattern = path.posix.join(themesDir.replaceAll('\\', '/'), '**/*.ts')
