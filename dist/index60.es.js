import { jsx as e } from "./index14.es.js";
//#region src/components/Icon/glyphs/chevronUp.tsx
var t = ({ variant: t }) => {
	switch (t) {
		case "downscaled": return /* @__PURE__ */ e("svg", {
			width: "16",
			height: "16",
			viewBox: "0 0 16 16",
			fill: "none",
			xmlns: "http://www.w3.org/2000/svg",
			children: /* @__PURE__ */ e("path", {
				d: "M6.23242 8.85164C6.0372 9.04685 5.72066 9.04677 5.52539 8.85164C5.33013 8.65637 5.33013 8.33987 5.52539 8.1446L8 5.67L10.4746 8.1446C10.6699 8.33987 10.6699 8.65637 10.4746 8.85164C10.2793 9.04673 9.96278 9.04684 9.76758 8.85164L8 7.08406L6.23242 8.85164Z",
				fill: "currentColor"
			})
		});
		case "default": return /* @__PURE__ */ e("svg", {
			width: "24",
			height: "24",
			viewBox: "0 0 24 24",
			fill: "none",
			xmlns: "http://www.w3.org/2000/svg",
			children: /* @__PURE__ */ e("path", {
				d: "M10.2342 12.8516C10.0389 13.0469 9.72144 13.0469 9.52618 12.8516C9.33127 12.6564 9.33123 12.3398 9.52618 12.1446L12.0018 9.66996L14.4764 12.1446C14.6716 12.3398 14.6716 12.6563 14.4764 12.8516C14.2811 13.0469 13.9646 13.0469 13.7693 12.8516L12.0018 11.084L10.2342 12.8516Z",
				fill: "currentColor"
			})
		});
		case "upscaled": return /* @__PURE__ */ e("svg", {
			width: "24",
			height: "24",
			viewBox: "0 0 24 24",
			fill: "none",
			xmlns: "http://www.w3.org/2000/svg",
			children: /* @__PURE__ */ e("path", {
				"fill-rule": "evenodd",
				"clip-rule": "evenodd",
				d: "M4.17524 15.8455C3.95071 15.6331 3.94056 15.2782 4.15278 15.0535L11.5922 7.17556C11.6979 7.06379 11.8447 6.99996 11.9985 6.99978C12.1524 6.99978 12.2999 7.06375 12.4057 7.17556L19.8452 15.0535C20.0575 15.2783 20.0474 15.6331 19.8227 15.8455C19.5979 16.0578 19.2431 16.0477 19.0307 15.823L11.9995 8.37575L4.96723 15.823C4.75489 16.0478 4.40009 16.0578 4.17524 15.8455Z",
				fill: "currentColor"
			})
		});
		default: throw Error(`chevronUp icon error: Unsupported combination variant=${t}`);
	}
};
//#endregion
export { t as chevronUp };
